...
preproc:
  ...
  - interpolate_bads:     {}
  - drop_channels:        {ch_names: ['MRC63-4122'], on_missing: ignore}
  ...
  - preproc_zapline_dss:  {'fline': 50, 'nremove': 4}
...
basedir = '/rds/projects/q/quinna-spectral-changes-in-ageing/mrc_meguk/raw_data/Nottingham/'
oxfpath = os.path.join(basedir, 'sub-{subj}', 'meg' ,'sub-{subj}_task-{task}_meg.ds')
st = osl.utils.Study(oxfpath)

inputs = st.get(task='resteyesopen') + st.get(task='resteyesclosed')
extrafuncs = [run_first_level_new]

outdir = '/rds/projects/q/quinna-spectral-changes-in-ageing/mrc_meguk/processed_data/Nottingham_firstlevel/'
