...
def _merge_grad_data(data, method="rms"):
    ...
    if method == "mean":
        ...
        # Reconstruct the data using the largest singular value component
        U, S, Vt = np.linalg.svd(data, full_matrices=False)
        data = S[0] * np.outer(U[:, 0], Vt[:, 0])
    else:
...
mode = 2
if mode == 0:
    # Normal processing with everything
    basedir = '/rds/projects/q/quinna-camcan/cc700/meg/pipeline/release005/BIDSsep/derivatives_rest/aa/AA_movecomp_transdef/aamod_meg_maxfilt_00003/'
    cc_path = os.path.join(basedir, 'sub-{subj}', 'mf2pt2_sub-{subj}_ses-rest_task-rest_megtransdef.fif')
    st = osl.utils.Study(cc_path)
    outdir = '/rds/projects/q/quinna-camcan/camcan_bigglm/processed-data/CamCAN_firstlevel'
    config['preproc'][-1]['run_first_level_new']['covs'] = True

elif mode == 1:
    # Normal GLM no maxfilter trans
    basedir = '/rds/projects/q/quinna-camcan/cc700/meg/pipeline/release005/BIDSsep/derivatives_rest/aa/AA_movecomp/aamod_meg_maxfilt_00002/'
    cc_path = os.path.join(basedir, 'sub-{subj}', 'mf2pt2_sub-{subj}_ses-rest_task-rest_meg.fif')
    st = osl.utils.Study(cc_path)
    outdir = '/rds/projects/q/quinna-camcan/camcan_bigglm/processed-data/CamCAN_firstlevel_movecomp'
    config['preproc'][-1]['run_first_level_new']['covs'] = True

elif mode == 2:
    # Normal GLM no maxfilter movecomp
    basedir = '/rds/projects/q/quinna-camcan/cc700/meg/pipeline/release005/BIDSsep/derivatives_rest/aa/AA_nomovecomp/aamod_meg_maxfilt_00001/'
    cc_path = os.path.join(basedir, 'sub-{subj}', 'mf2pt2_sub-{subj}_ses-rest_task-rest_meg.fif')
    st = osl.utils.Study(cc_path)
    outdir = '/rds/projects/q/quinna-camcan/camcan_bigglm/processed-data/CamCAN_firstlevel_nomove'
    config['preproc'][-1]['run_first_level_new']['covs'] = True

elif mode == 3:
    # No covs GLM with maxfilter
    basedir = '/rds/projects/q/quinna-camcan/cc700/meg/pipeline/release005/BIDSsep/derivatives_rest/aa/AA_movecomp_transdef/aamod_meg_maxfilt_00003/'
    cc_path = os.path.join(basedir, 'sub-{subj}', 'mf2pt2_sub-{subj}_ses-rest_task-rest_megtransdef.fif')
    st = osl.utils.Study(cc_path)
    outdir = '/rds/projects/q/quinna-camcan/camcan_bigglm/processed-data/CamCAN_firstlevel_nocovs'
